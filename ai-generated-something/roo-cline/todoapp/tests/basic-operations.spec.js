...
test.describe('基本的なタスク操作', () => {
  ...
  test('タスクを編集できる', async ({ page }) => {
    ...
    // タスク情報を編集
    await page.fill('#editTaskTitle', '編集後のタスク');
    await page.selectOption('#editTaskSize', 'small');
    await page.selectOption('#editTaskImportance', 'high');
  ...
  test('タスクのステータスを変更できる', async ({ page }) => {
    ...
    // 完了リストに移動していることを確認
    await page.waitForSelector('#completedTasks .task-item');
    const completedTask = await page.locator('#completedTasks .task-item h3 span').textContent();
